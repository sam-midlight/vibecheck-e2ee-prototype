...
App-agnostic zero-knowledge encryption primitives. Designed to be **lifted into another Next.js/Supabase app unchanged**. No React, no UI, no domain-specific types.

Matrix-aligned cross-signing (MSK / SSK / USK) + per-device key bundles + per-sender Megolm ratchet + SAS emoji verification.
...
Plus a Supabase (or equivalent) layer supplying the tables in `supabase/migrations/`. Apply them linearly to a fresh Postgres+Supabase project. Tables in use:
...
key_backup,
megolm_sessions, megolm_session_shares, key_forward_requests,
cross_user_signatures, sas_verification_sessions,
tos_acceptances,
calls, call_members, call_key_envelopes              (video-call scope)
```
...
| `device.ts`       | `UserMasterKey` + `DeviceKeyBundle` generation, device issuance + revocation certs (SSK-signed), chain verification, display-name sealing |
...
| `blob.ts`         | AEAD encrypt / decrypt + Ed25519 sig. v4 = Megolm; v3 = flat room key + device sig |
...
| `tofu.ts`         | Observe a contact's MSK pub; emit key-change events (compares ed25519 only)   |
...
| --------------------- | -------------------------------------------------------------------------- | --------------------------------------------------------- | ------------------------------------------------------------------------------------------------ |
| **MasterSigningKey**  | One Ed25519 keypair per user                                               | Recovery blob + original primary only. Stays cold.        | Signs SSK + USK cross-sigs. Never signs device certs directly.                                   |
| **SelfSigningKey**    | One Ed25519 keypair per user                                               | Every co-primary device (shared via sealed box on approval). | Signs device issuance + revocation certs.                                                    |
...
| **DeviceCertificate** | SSK Ed25519 signature over `(user_id, device_id, device_ed_pub, device_x_pub, createdAtMs)` | Stored in `devices` row | Proves this device belongs to this user. Chains `SSK ← MSK cross-sig ← MSK`.                    |

`identities.ed25519_pub` is the MSK pub. `UserMasterKey` is a type alias for `MasterSigningKey`.

**Two cert-signing paths exported:** `signDeviceIssuanceV2` (SSK-signed, domain `vibecheck:devcert:v2`) and `signDeviceIssuance` (MSK-signed, domain `vibecheck:devcert:v1`). New code writes SSK-signed certs. `verifyPublicDevice(device, mskPub, sskPub?)` tries v2 first when `sskPub` is supplied, falls back to v1 — so identities whose MSK has no published SSK still verify. The same split applies to revocation certs (`signDeviceRevocationV2` / `signDeviceRevocation`).

**Verifier rule:** when `sskPub` is available, always call `verifyCrossSigningChain` before trusting an SSK-signed cert. `verifyPublicDevice` does this internally.
...
```ts
import { encryptBlob, encryptBlobV4, decryptBlob } from '@/lib/e2ee-core';

// Flat-room-key write (v3) — used for bootstrap / attachments / pair-room
// pre-Megolm messages:
const blob = await encryptBlob({
  ...
  senderDeviceEd25519PrivateKey: bundle.ed25519PrivateKey,
});

// Megolm write (v4) — used for room messages once a Megolm session is seeded:
const v4 = await encryptBlobV4({
  payload: { type: 'message', text: 'hi' },
  roomId,
  messageKey, sessionId, messageIndex,       // from ratchetAndDerive
  senderUserId, senderDeviceId,
  senderDeviceEd25519PrivateKey: bundle.ed25519PrivateKey,
});
// ship {nonce, ciphertext, generation, sessionId?, messageIndex?} to the server.
...
const decoded = await decryptBlob<Payload>({
  ...
  roomId,
  roomKey,                                   // for v3 path
  resolveSenderDeviceEd25519Pub,             // (userId, deviceId) → ed pub
  resolveMegolmKey,                          // (sessionId, index) → messageKey (v4)
});
```

Envelope versions:
- **v4 (Megolm)** — `payload` sealed under a `messageKey` derived from a per-sender ratchet. Envelope carries `sid + mi` (session_id + message_index). Verifier derives the matching key from its inbound snapshot and verifies the device-ed signature inside the AEAD. AD = `room_id(16) || session_id(32) || message_index(4 BE)`. Sig covers `domain || room_id || session_id || message_index || nonce || payloadBytes`.
- **v3 (flat room key + device sig)** — signature inside AEAD, signed by the sender's device Ed25519 priv. AD = `room_id(16) || generation(4 BE)`. Sig covers `domain || room_id || generation || nonce || payloadBytes`.
- `decryptBlob` also accepts **v2** envelopes (user-root-signed inside AEAD) for in-wire data written before per-device sigs. New code writes only v3/v4.

Distinct domain tag per version (`vibecheck:blob:v{2,3,4}`) prevents cross-version confusion.
...
**Server-side rotation safety net:** a BEFORE-INSERT trigger on `blobs` rejects inserts whose session has ≥200 messages. A BEFORE-UPDATE trigger on `megolm_sessions` rejects `message_count` decreases unless `session_id` also changes (otherwise the sender could reset their own counter and loop the cap). The client is authoritative for rotation at 100 — the 100→200 gap accommodates rotation races. Do NOT bypass either layer by writing directly via service-role.

## SAS verification

Interactive emoji-based identity verification between two users. On success, each side's USK cross-signs the other's MSK pub → `cross_user_signatures`. Verified contacts get an escalated key-change alert if their MSK ever drifts.
...
Emoji set is 64 animals/plants/symbols in `SAS_EMOJI`. 7 emoji × 6 bits = 42 bits of transcript entropy. `SAS_EMOJI` is part of the protocol definition; do not reorder without versioning the domain tag.

## Room keys (rotation + approval + backup)

Megolm handles blob encryption, but room keys are still used for: wrapping during rotation + approval, encrypted room-name sealing (`encryptRoomName` / `decryptRoomName`), and pair-room bootstrap before the first Megolm session.
...
QR-based secondary-device flow. This app uses the approval flow (`approval.ts`) as the primary linking path.
...
24-word BIP-39 phrase wraps the MSK priv + backup key via Argon2id. The phrase never leaves the client. Entering the phrase on a fresh device unwraps both, so history restore fires automatically post-enrollment.
...
Blob payload: `[ MSK_priv(64) || backupKey(32) ]` (96 bytes). AD = `vibecheck:recovery:v3:${userId}`.

`normalizePhrase` is lenient — strips `1.`, `1)`, `(1)`, `1:` prefixes and collapses whitespace. Argon2id `opslimit` + `memlimit` are stored per row, so parameters can be raised for new users without orphaning existing phrases.

## Key backup (Matrix key-backup)

Every room key + Megolm snapshot the user holds is encrypted under the 32-byte `backupKey` and uploaded to `key_backup`. The backup key lives inside the recovery blob, so phrase entry on a fresh device recovers BOTH the MSK priv AND the backup key in one shot.
...
AD = `vibecheck:key-backup:v1:${roomId}:${generation}` — distinct from every other AD tag in the app.

**Three paths by which a device gets the backup key:**
1. **Primary first-phrase setup** — `generateBackupKey()` in `RecoveryPhraseModal`, stashed via `putBackupKey` + baked into the recovery blob.
2. **New device via recovery phrase** — `unwrapUserMasterKeyWithPhrase` returns `{ ed25519PrivateKey, backupKey }`; callback writes the backup key locally BEFORE `enrollDeviceWithUmk`.
3. **New device via approval flow** — A seals the backup key to B's x25519 pub, writes to `devices.backup_key_wrap`. B's auth callback unseals post-enrollment and calls `putBackupKey`.
...
- One browser assumption: `storage.ts` needs `indexedDB`. Everything else works in any JS env where libsodium loads.
- No plain `Error`s from the module. Callers discriminate via `CryptoError.code` and `TrustError.code`.
- **IndexedDB versioning:** the module's `openDB` calls all migrations idempotently from v0. Any downstream app that bumps the version must preserve the existing upgrade path.

## Porting into another app

1. Copy the `e2ee-core/` folder and ALL migrations verbatim.
2. Install `libsodium-wrappers-sumo`, `idb`, and `@scure/bip39` in the target app. Also `livekit-client` if porting the video-call surface (`e2ee-core/call.ts` + `src/lib/livekit/`).
3. In the target app, implement a thin data layer equivalent to `src/lib/supabase/queries.ts` (or reuse it). The contract: for every table, a function to insert, fetch, list, delete, and subscribe; all byte columns base64-encoded on the wire. **RLS + ON CONFLICT trap**: any "sealed row addressed to a peer device" table (`megolm_session_shares`, plausibly future tables) MUST use plain `.insert()` + 23505 swallow, never `.upsert()`.
4. Wire up the flows:
   ...
   - **callback** → `supabase.auth.getSession()` first, then decide on: returning / passphrase-locked / new-device chooser / first-sign-in.
   ...
   - **link a second device** → approval flow (B creates `device_approval_requests`; A verifies the code + writes the device row with signed cert + `devices.signing_key_wrap` + `devices.backup_key_wrap`; B reads all three, unseals, calls `putSelfSigningKey`/`putUserSigningKey`/`putBackupKey`).
...
5. Keep the Supabase client on `flowType: 'implicit'` unless the target is SSR-first with `@supabase/ssr` cookie storage.
6. Bring across `StatusCheck` / `status/page.tsx` if you want the live verification dashboard — it's a strong regression harness. Add one check per E2EE-touching feature the consuming app adds.

None of the e2ee-core functions are specialized to this repo's schema — they operate on keys and bytes. The discriminated-union payload type and any `zod` validation at the decrypt boundary live in the downstream app, NOT in e2ee-core.
